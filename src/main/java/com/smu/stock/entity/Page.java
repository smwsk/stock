...
import com.smu.stock.utils.Tools;
...
public class Page {
	...
	public Page() {
		try {
			this.showCount = Integer.parseInt("10");
		} catch (Exception e) {
			this.showCount = 15;
		}
	}
	...
	public String getPageStr() {
		...
		if (totalResult > 0) {
			...
			if (currentPage == 1) {
				sb.append("	<li style=\"width:75px;\"><a>共<font color=red>"
						+ totalResult + "</font>条</a></li>\n");
			...
			sb.append("}\n");
			sb.append("</script>\n")
					.append("<style >.list-inline li {float:left;padding-right:10px;}</style>");
		}
		pageStr = sb.toString();
		return pageStr;
	}
